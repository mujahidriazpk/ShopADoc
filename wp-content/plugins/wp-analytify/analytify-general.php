...
define( 'ANALYTIFY_VERSION', '5.0.0' );
	...
	class Analytify_General {
		...
		public function get_ga_properties() {
			...
			try{
				...
				WPANALYTIFY_Utils::add_ga4_exception( 'fetch_ga4_profiles_exception', $error_message, $error_message );
				WPANALYTIFY_Utils::handle_ga4_exceptions( $this->get_ga4_exception() );
				return array();
			}
			$ga_properties = array();
		...
		public function get_reports( $name, $metrics, $date_range, $dimensions = array(), $order_by = array(), $filters = array(), $limit = 0, $cached = true ) {
			...
			if ( $cached ) {
				set_transient( $cache_key, $formatted_reports, $this->transient_timeout );
			}
		...
		public function create_dimension( $parameter_name, $display_name, $scope, $description = '', $property_id = '' ) {
			...
			try {
				...
				try { // Try to create new dimension.
					...
					$logger = analytify_get_logger();
					$logger->warning( $th->getMessage(), array( 'source' => 'analytify_create_dimensions_exception' ) );
					$return_response = array(
						'response' => 'failed',
						'code'     => $response->code,
