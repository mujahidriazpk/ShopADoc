...
if ( ! class_exists( 'Analytify_MP_GA4' ) ) {
	...
	class Analytify_MP_GA4 {
		...
		private function __construct() {
			...
			$this->api_secret     = $wp_analytify->settings->get_option( 'measurement_protocol_secret', 'wp-analytify-advanced', false );
			$this->measurement_id = WP_ANALYTIFY_FUNCTIONS::get_UA_code();
			$this->client_id = ( 'on' === $wp_analytify->settings->get_option( 'user_advanced_keys','wp-analytify-advanced' ) ) ? $wp_analytify->settings->get_option( 'client_id','wp-analytify-advanced' ) : ANALYTIFY_CLIENTID;
		}
		...
		public function send_hit( $events ) {
			...
			if ( $debug_mode ) {
				foreach ( $events as $index => $event ) {
					$events[ $index ]['params']['debug_mode'] = true;
				}
			}
