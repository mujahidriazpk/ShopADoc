...
/*	if(isset($auction_city) && $auction_city !="" ){
	...
	if($period=='today' || $period=='yesterday'){
		...
		if(isset($auction_zip_code) && $auction_zip_code !="" ){
			//echo $start."==".$end."<br />";
			//echo $product_query->request."<br /><br /><br /><br /><br />";
		}
		...
		$relist_count =0;
		if($count > 0){
			$query_relist_count = "select meta_id from `wp_postmeta` where meta_key='_auction_relist_expire' and post_id in ('".implode("','",$post_ids)."') and meta_key='yes'";
			//echo $query;
			$wpdb->get_results($query_relist_count);
    		$relist_count = $wpdb->num_rows;
			$success_count = $count - $relist_count;
			/*foreach($posts as $post){
				$_auction_relist_expire = get_post_meta($post->ID, '_auction_relist_expire',TRUE);
				if($_auction_relist_expire){
					$relist_count++;
