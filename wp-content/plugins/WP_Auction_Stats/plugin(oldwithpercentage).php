	...
	public static function get_dates_of_quarter($quarter = 'current', $year = null, $format = 'Y-m-d')
{
	...
	public static function getAuctionCustomFunc($auction_state,$mishaDateFrom,$mishaDateTo,$period){
	...
	public static function getUserCustomFunc($user_state,$mishaDateFrom,$mishaDateTo,$period){
		global $wpdb;
...
// Output: 7.2k
	...
	public function plugin_settings_page() {
		...
		if($period_vistor=='today'){
			...
			$quarter_vistor = Post_Stats_List::get_dates_of_quarter('','','m/d/y');
		...
		if($period=='today'){
			...
			$quarter = Post_Stats_List::get_dates_of_quarter('','','m/d/y');
			$from = $quarter['start'];
			$to = $quarter['end'];
		}
		/*if($period=='today' || $period=='yesterday')
    ...
    <div id="post-body" class="metabox-holder">
      <div id="post-body-content">
        <div class="meta-box-sortables ui-sortable"> 
						...
						jQuery('#map').usmap({
							...
							'click' : function(event, data) {
										...
										success:function (data){
											if(data!=""){
												window.location.replace("<?php echo home_url();?>/wp-admin/admin.php?page=auction_state_performance&map_id="+data);
											}
										}
								});
							}
          ...
          <div id="details" >
            <div id="filter_date" >
              <div class="heading"><span class="heading1">PERFORMANCE -</span><span class="heading2">&nbsp;NATIONWIDE</span></div>
              <?php 
					/*$from = ( isset( $_POST['mishaDateFrom'] ) && $_POST['mishaDateFrom'] ) ? $_POST['mishaDateFrom'] : '';
					$to = ( isset( $_POST['mishaDateTo'] ) && $_POST['mishaDateTo'] ) ? $_POST['mishaDateTo'] : '';*/
					global $US_state;
              ...
              <!--<input type="submit" name="submit[filter]" value="Filter" class="btn btn-primary btn-filter" style="padding: 5px 9px !important;width:12%;" />-->
              <?php 
					$item_vistor = Post_Stats_List::getAuctionCustomFunc('',$from_vistor,$to_vistor,$period_vistor);
					$item = Post_Stats_List::getAuctionCustomFunc('',$from,$to,$period);
					$item_User = Post_Stats_List::getUserCustomFunc('',$from_vistor,$to_vistor,$period_vistor);
					//$number = SP_Plugin_Post_Stat::number_format_short(7200001);
              ...
              <div class="top_panel top_panel_left">
                <table width="100%" style="margin-top:10px;" class="table_bg">
                  ...
                  <tbody >
                    <tr>
                      <td><strong class="amt font-22"><?php echo SP_Plugin_Post_Stat::number_format_short($item_User['total']);?></strong></td>
                      <td><strong class="amt font-22"><?php echo SP_Plugin_Post_Stat::number_format_short($item_vistor['no_auctions']);?></strong></td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <table width="100%" class="scroll wp-list-table widefat fixed striped table-view-list posts" id="dest" style="width:100%;height:630px;overflow:scroll;">
              ...
              <tbody >
                <?php unset($US_state['District of Columbia']);$i=1;foreach($US_state as $k=>$v){
								//echo "'".$v."',";
								//$types = array('126'=>'Auction Listing Fee','1141'=>'Registration Fee','948'=>'Subscription Fee','942'=>'Auction Cycle fee','1642'=>'Auction Relisting Fee',);
								...
								$item_vistor = '';
								$item_User='';
								$item_vistor = Post_Stats_List::getAuctionCustomFunc($k,$from_vistor,$to_vistor,$period_vistor);
								$item = Post_Stats_List::getAuctionCustomFunc($k,$from,$to ,$period);
								$item_User = Post_Stats_List::getUserCustomFunc($k,$from,$to ,$period);
								$total_client = $item_User['client'];
								$returning = $item['returning'];
								$percentage_returning = 0;
								if($returning>0){
								...
								echo '<span id="tooltip-html-'.strtoupper($v).'" style="display:none;"><p><strong>PERFORMANCE - <span class="State_name">'.strtoupper($k).'</span></strong></p><p><p style="float:left;width:18%;"><strong>Visitors</strong><br /><strong class="amt">'.SP_Plugin_Post_Stat::number_format_short($item_User['total']).'</strong></p><p style="float:left;width:22%;"><strong>Auctions</strong><br /><strong class="amt">'.SP_Plugin_Post_Stat::number_format_short($item_vistor['no_auctions']).'</strong></p><p style="float:left;width:22%;"><strong>Success</strong><br /><strong class="amt">'.round($item['success']).'%</strong></p><p style="float:left;width:18%;"><strong>Relists</strong><br /><strong class="amt">'.round($item['relist']).'%</strong></p><p style="float:left;width:20%;"><strong>Returning</strong><br /><strong class="amt">'.$percentage_returning.'%</strong></p></p></span>';
								echo '<tr id="over-'.$v.'">';
									echo '<td align="left"><strong>'.$k.'</strong></td>';
									echo '<td align="left" class=""><strong>'.SP_Plugin_Post_Stat::number_format_short($item_User['total']).'</strong></td>';
									echo '<td align="left" class=""><strong>'.SP_Plugin_Post_Stat::number_format_short($item_vistor['no_auctions']).'</strong></td>';
