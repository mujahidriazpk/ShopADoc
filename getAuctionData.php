...
if(isset($_REQUEST['mode']) && $_REQUEST['mode']=='checkBidUpdate'){
	$return = null;
	 if ( isset( $_REQUEST['product_id'] ) && $_REQUEST['product_id']  !="" ) {
			...
			if($curent_bid !=$_REQUEST['current_bid'] && $current_bider !=""){
				...
				$_auction_current_bid = get_post_meta($_REQUEST['product_id'], '_auction_current_bid', true );
	...
	if(str_replace(",","",$_auction_start_price)  != $_REQUEST['price'] ){
		if($_REQUEST['seller_screen'] == 'yes'){
			if( $_REQUEST['seller_screen_update'] == 'no'){
			 $return['price'] = number_format(round(str_replace(",","",$_auction_start_price)),0,",",",");
			 $return['new_price'] = round(str_replace(",","",$_auction_start_price));
			 $return['add_to_cart_text'] = 'update_price_seller';
			 $return['response_mode'] = 'update_price_seller';
