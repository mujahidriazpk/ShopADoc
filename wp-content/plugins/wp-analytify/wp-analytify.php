	...
	class WP_Analytify extends Analytify_General {
		...
		public function admin_scripts( $page ) {
			...
			if ( 'toplevel_page_analytify-dashboard' === $page ) {
				...
				if ( class_exists( 'WP_Analytify_Pro_Base' ) && ( ! isset( $_GET['show'] ) || 'detail-realtime' !== sanitize_text_field( $_GET['show'] ) ) ) {
				...
				if ( in_array( 'show-overall-dashboard', $show_settings ) ) {
					if ( 'ga4' === WPANALYTIFY_Utils::get_ga_mode() ) {
						...
						$stats        = $wp_analytify->get_reports(
							'analytify-single-general-stats',
							array(
								'sessions',
								'totalUsers',
								'screenPageViews',
		 ...
		 * @since 2.0.0
		 */
		public function track_miscellaneous_errors() {
			$is_tracking_available = WPANALYTIFY_Utils::is_tracking_available();
...
function wp_analytify_activate() {
	...
	if ( $_admin_settings && 'on' === $_admin_settings['disable_back_end'] && ! empty( $_admin_settings['show_analytics_roles_back_end'] ) ) {
		return;
	}
